...
if __name__ == '__main__':
    ...
    stream = CamGear(source='https://www.youtube.com/watch?v=G0IBqtO1K28', y_tube =True).start() # YouTube Video URL as input
 
    # frames = 0
    # start = time.time()
    while True(): #カメラが起動している間
 
        ret, frame = stream.read() #キャプチャ画像を取得
        if ret:
            # 解析準備としてキャプチャ画像を加工
